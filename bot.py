...
Also includes /generate_session — logs in with a phone number (OTP, and
2FA password if needed) to generate a fresh Telethon session string for
any account you own, without needing a separate bot.
...
# separate state just for the /generate_session login flow (phone/code/password)
gs_state: dict[int, dict] = {}
...
MAIN_MENU = [
    ...
    [Button.inline("🔑 Generate New Session", "gs_start")],
...
async def start_handler(event):
    if not logged_in():
        ...
        await event.respond(
            ...
            "Don't have one? Send /generate_session instead and I'll walk you "
            "through logging in (phone → OTP → 2FA if needed) to get one."
        )
        return
...
# ---------------------------------------------------------------------------
# /generate_session — generate a fresh session string from inside this bot
# ---------------------------------------------------------------------------

async def gs_cleanup(user_id):
    st = gs_state.pop(user_id, None)
...
@bot.on(events.NewMessage(pattern="/generate_session"))
@owner_only
async def generate_session_start(event):
    await gs_cleanup(event.sender_id)
    gs_state[event.sender_id] = {"step": "phone"}
    await event.respond(
        "🔑 **Session Generator**\n\n"
        "This logs in to a Telegram account and gives you back a fresh "
        "session string — for this account, or any other account you own.\n\n"
        "⚠️ Only use your own phone number.\n\n"
        "Send the phone number now, international format, e.g. `+919876543210`",
        buttons=[[Button.inline("❌ Cancel", "gs_cancel")]],
    )


@bot.on(events.CallbackQuery(data=b"gs_start"))
@owner_only
async def gs_start_button(event):
    await gs_cleanup(event.sender_id)
    gs_state[event.sender_id] = {"step": "phone"}
    await event.edit(
        "🔑 **Session Generator**\n\n"
...
@bot.on(events.CallbackQuery(data=b"gs_cancel"))
@owner_only
async def gs_cancel_handler(event):
    await gs_cleanup(event.sender_id)
    await event.edit("❌ Cancelled.", buttons=MAIN_MENU if logged_in() else None)


async def gs_handle_phone(event, st):
    ...
    msg = await event.respond("📲 Sending OTP...")
    ...
    try:
        ...
        await msg.edit("❌ Invalid phone number. Send /generate_session to try again.")
        await client.disconnect()
        gs_state.pop(event.sender_id, None)
        return
    except FloodWaitError as e:
        await msg.edit(f"⏳ Too many attempts. Try again in {e.seconds}s.")
        await client.disconnect()
        gs_state.pop(event.sender_id, None)
        return
    except Exception as e:
        await msg.edit(f"❌ Error: `{e}`\nSend /generate_session to try again.")
        await client.disconnect()
        gs_state.pop(event.sender_id, None)
        return
    ...
    await msg.edit(
        "✅ OTP sent to that account's Telegram app.\n\n"
        "Send the code you received — just the digits, e.g. `12345` "
        "(no dashes, even if shown as `1-2-3-4-5`)."
    )


async def gs_handle_code(event, st):
    ...
    try:
        ...
        await msg.edit("❌ Wrong code. Send the correct code again, or /generate_session to restart.")
        return
    ...
    await gs_finish(event, msg, st)


async def gs_handle_password(event, st):
    ...
    try:
        ...
        await msg.edit("❌ Wrong password. Try again, or /generate_session to restart.")
        return
    except Exception as e:
        await msg.edit(f"❌ Error: `{e}`\nSend /generate_session to try again.")
        await gs_cleanup(event.sender_id)
...
async def gs_finish(event, msg, st):
    client = st["client"]
    ...
    await msg.edit(
        f"✅ **Login successful!**\n"
        f"Account: **{display_name(me)}** (@{me.username or 'no_username'})\n\n"
        f"Your session string is below 👇\n"
        f"⚠️ Treat this like a password. Save it somewhere safe, then "
        f"delete both this message and the one below.\n\n"
        f"To control THIS account with the bot, send /login <the string below> "
        f"(or paste it after /start if not logged in yet)."
    )
    await event.respond(f"`{session_string}`")
    await event.respond(
        "🗑 Delete the message above once you've saved it (tap and hold → Delete). "
        "If it ever leaks, go to Telegram Settings → Devices and terminate that session."
    )

    await client.disconnect()
    gs_state.pop(event.sender_id, None)
    await event.respond("Back to menu:", buttons=MAIN_MENU if logged_in() else None)


@bot.on(events.NewMessage)
@owner_only
async def free_text_handler(event):
    """Handles replies when bot is waiting for session login, generate_session
    steps, message-to-send, or ID-to-delete."""
    if event.raw_text.startswith("/"):
        return

    # /generate_session flow takes priority — it has its own state dict
    if event.sender_id in gs_state:
        st = gs_state[event.sender_id]
        step = st["step"]
        if step == "phone":
            await gs_handle_phone(event, st)
        elif step == "code":
            await gs_handle_code(event, st)
        elif step == "password":
            await gs_handle_password(event, st)
        return
...
async def callback_handler(event):
    ...
    if not logged_in() and data not in ("menu", "gs_start", "gs_cancel"):
